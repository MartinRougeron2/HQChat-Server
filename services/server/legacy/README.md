...
| `e2e.test.ts`, `account-delete.e2e.test.ts`, `test-client.ts` | the end-to-end suite (needs Postgres) |
...
```bash
npm run test:e2e        # the reason this directory exists (needs DATABASE_URL)
