...
export const StripeService = {
  ...
  async createCheckout(baseUrl: string): Promise<string> {
    const session = await stripeClient().checkout.sessions.create({
      ...
      // Always create a customer: `customer.subscription.*` events carry a
      // customer id and nothing else we can resolve, so a guest checkout would
      // leave later renewals and cancellations unattributable.
